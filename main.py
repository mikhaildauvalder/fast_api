...
async def get_recipe(session: SessionDep):
    query = select(RecipeModel.title, RecipeModel.views,
                   RecipeModel.cooking_time).order_by(desc(RecipeModel.views),
                                                      asc(RecipeModel.cooking_time))
...
@app.get('/recipes/{recipe_id}',
         tags=['Рецепты'],
         summary='Получить один конкретный рецепт')
async def get_spec_recipe(recipe_id, session: SessionDep):
    query = select(RecipeModel).where(RecipeModel.id == recipe_id)
    counter_query = update(RecipeModel).values(views=RecipeModel.views + 1).where(RecipeModel.id == recipe_id)
